...
export default class Application extends Component {
  constructor() {
    ...
    this.state = {
      ...
      newMessage: '',
      user: '',
      currentUser: '', //pass to message, iterate thru messages, if user = current user, push into currentUser Array. If button clicked, display from the array.
  ...
  handleUserFilter(userToFilter){
    if (this.state.filteredUser === userToFilter) {
      return this.setstate({filteredUser:[], currentUser: ''});
    }
    return (this.setState({
      filteredUser: userToFilter, currentUser: (this.state.messages.filter(m => m.user.user === user)),
    })
  );
  }
        ...
        <div className="user-info-container">
          ...
          <p>
            {uniqueUsers.map(user =>
              <button className="users-present-button" onClick={() => filterUser(user)}>{user}</button>
            )}
          </p>
        </div>
