...
export default class Application extends Component {
  ...
  hideItem(logInButton, clearMessageButton, logOutButton){
    logInButton.setAttribute('hidden', 'true');
    clearMessageButton.setAttribute('hidden', 'true');
  ...
  loadMessages(){
    ...
    console.log('Load: ', this.state.messages);
  } //end of loadMessages
  ...
  onAuthStateChanged(user){
    ...
    if (user) {
      ...
      this.showItem(items.clearMessageButton, items.logOutButton, items.messageEntryField, items.submitMessageButton);
      this.hideItem(items.logInButton);
    }

    else {
    //user is not logged in
    this.hideItem(items.clearMessageButton, items.logOutButton);
    this.showItem(items.logInButton);
    //**stopped
  ...
  showItem(clearMessageButton, logOutButton, messageEntryField, submitMessageButton, logInButton){
    clearMessageButton.removeAttribute('hidden');
    logOutButton.removeAttribute('hidden');
    messageEntryField.removeAttribute('hidden');
    submitMessageButton.removeAttribute('hidden');
...
//Gabi: do you want to get rid of this?
  ...
  findMatchingMessages() {
    ...
    let result = this.state.messages.filter(
        (message) => {
          return message.body.indexOf(this.state.search) !== -1;
        }
    );
    console.log('result ', result);
    this.setState({messages: result});
  }
//onchange of input field may need to change, not grabbing input for the first element
// make sure searching with the full list of messages
...
class ChatMaster extends React.Component {
  ...
  render () {
    ...
    const searchedMessages = this.props.messages.filter(
        (message) => {
          return message.body.indexOf(this.state.searchFilter) !== -1;
        }
    )
    return (
      <div>
