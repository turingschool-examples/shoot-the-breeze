...
export default class Application extends Component {
  ...
  loadMessages(){
    reference.limitToLast(25).on('value', (snapshot) =>  {
      ...
      this.setState({
        ...
        // uniqueArray: this.uniqueUserArray(messagesReturned)
      });
  ...
  saveMessage(){
    if (this.state.newMessage !== '' && this.isUserSignedIn()) {
      ...
      // const newMessages = currentMessages.concat([newMsgObj]);
  ...
  render(){
    return (
      <div onLoad={this.initFirebase()}>
        ...
        <div className="body">
          <ChatMaster messages={this.state.messages} usersInChat={this.state.uniqueArray}/>
        </div>
        <footer>
        ...
        <div className="user-info-container">
          ...
          <p>
            {uniqueUsers.map(user =>
              <button onClick={() => this.filterUserMessages(user)}>{user}</button>
            )}
          </p>
