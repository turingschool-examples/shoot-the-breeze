...
export default class Application extends Component {
  ...
  UpdateFilteredState(){
    this.setState(
      {filteredMessages: _.filter(this.state.messages,
        (message) => {
          return messages.content.includes(message);
      })}
    );
  }

  FilterArray(){
    _.filter(this.state.messages() =>{
      return messages.content.toLowerCase().includes(message);
    });
  }
