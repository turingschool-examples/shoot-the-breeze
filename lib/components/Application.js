...
export default class Application extends Component {
  ...
  handleInputChange(e){
    ...
    const items = {
      clearButton: document.querySelector('.clear-message-button'),
      submitButton: document.querySelector('.submit-message-button'),
    ...
    if (items.thereIsANewMessage) {
      ...
      this.enableButtons();
      // items.submitButton.removeAttribute('disabled');
      // items.clearButton.removeAttribute('disabled');
    }

      if (this.state.newMessage.length >= 140) {
      items.submitButton.setAttribute('disabled', 'true');
    }
